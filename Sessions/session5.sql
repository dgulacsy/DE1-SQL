...
## Need to drop procedure to run a new one
...
>>>>>>> 79cb74fc4a1ed792d12bfa4537332af59e2a4185:SQL5/session5.sql
DELIMITER //
...
DROP PROCEDURE IF EXISTS GetPaymentAmount;

DELIMITER $$

CREATE PROCEDURE GetPaymentAmount (
	IN  x INTEGER,
	OUT xamount int
)
BEGIN
...
CALL GetPaymentAmount(1,@xamount);
SELECT @xamount;
...
DROP PROCEDURE IF EXISTS GetPaymentLevel;

DELIMITER $$

CREATE PROCEDURE GetPaymentLevel(
    	IN  rowNumber INT, 
    	OUT paymentLevel VARCHAR(20)
)
BEGIN
	DECLARE pAmount DECIMAL DEFAULT 0;
	...
	IF pAmount > 100000 THEN
		SET paymentLevel = 'CAT1';
	ELSEIF pAmount > 10000 THEN
		SET paymentLevel = 'CAT2';
	ELSE
		SET paymentLevel = 'CAT3';
	END IF;
END$$
DELIMITER ;

CALL GetPaymentLevel(4, @paymentLevel);
SELECT @paymentLevel, amount FROM payments LIMIT 1;
